...
class LicenciaConformidad(models.Model):
    _name = 'licencia.conformidad'
    _description = "Licencia de Conformidad"
    ...
    def _get_report_base_filename(self):
        self.ensure_one()
        return '%s %s' % ('Licencia de Conformidad', self.name)

    def _compute_access_url(self):
        # super(SolicitudesServicio, self)._compute_access_url()
        for verificacion in self:
            verificacion.access_url = '/my/licencia-conformidad/%s' % (verificacion.id)
    ...
    def button_print(self):
        return self.env.ref('intn_trazabilidad_uso_marca.reporte_licencia_conformidad').report_action(self)
    ...
    def generate_qr_code(self):
        for i in self:
            ...
            route = "/licencia_conformidad_check?licencia_conformidad_id=" + \
    ...
    def button_confirmar(self):
        for this in self:
            seq = self.env['ir.sequence'].sudo().next_by_code('seq_licencia_conformidad')
            this.write({'name': seq})
            this.write({'state': 'done'})
            reg = {
                'res_id': self.id,
                'res_model': 'licencia.conformidad',
    ...
    def _fechaVenicimientoLetras(self):
        ...
        for this in self:
            if this.fecha:
                ...
                fecha_vencimiento_letras = fecha_vencimiento_letras.replace('__mes__', meses[this.fecha_vencimiento.month - 1])
